...
def test_event_e2e_returns_grounded_answer():
    client = TestClient(create_app(E2EContainer()))
    response = client.post(
        "/v1/query",
        json={"event_input": "TSMC maintenance may affect packaging", "batch_id": "batch-20260313"},
    )
    body = response.json()
    assert response.status_code == 200
    assert body["citations"]
    assert body["trace_id"]
    assert "Event analysis" in body["final_response"]